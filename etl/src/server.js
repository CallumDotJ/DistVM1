...
const AMQP_URL = process.env.AMQP_URL || `amqp://${RMQ_USER}:${RMQ_PASS}@${RMQ_VM2_PRIVATEIP}:${RMQ_PORT}`;
...
async function processMessage(msgObj) { // etl function to process each message
  ...
  const type = String(msgObj.type || "").trim().toLowerCase();
  ...
  try {
    ...
    await conn.execute("INSERT IGNORE INTO tbl_type (type) VALUES (?)", [type]);
...
async function startConsumer() { // connect to RabbitMQ and start consuming messages
  ...
  channel.consume(QUEUE_NAME, async (msg) => { // callback for each message
    if (!msg) return;
